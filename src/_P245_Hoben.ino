...
//
// To have a real temperature sent, an external sensor in another task should be selected.
// It can be a real sensor or a dummy value 
...
#define PLUGIN_NAME_245 "Regulator - Hoben"
...
boolean Plugin_245(byte function, struct EventStruct *event, String &string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_CLOCK_IN:
      {
        addLog(LOG_LEVEL_DEBUG_MORE, F("HOBEN : CLOCK: "));
    ...
    case PLUGIN_READ:
      {
        if (Plugin_245_irSender == 0) {
          addLog(LOG_LEVEL_INFO, F("HOBEN : No IR LED, skipping: "));
          success = true;
          break;
        }

        // Buffer for HOBEN Pattern
        uint16_t *buf;
        ...
        addLog(LOG_LEVEL_DEBUG_MORE, F("HOBEN PLUGIN READ : "));
        ...
        addLog(LOG_LEVEL_INFO, F("HOBEN : IR Code sent "));
    ...
    case PLUGIN_WRITE:
      {
        addLog(LOG_LEVEL_DEBUG_MORE, F("HOBEN PLUGIN WRITE : "));
